...
function draw() {
  ...
  if (gameState===PLAY){
   ...
   console.log(kangaroo.y)
    ...
    if(obstaclesGroup.collide(kangaroo)){
      collidedSound.play();
      gameState = END;
    }
    //escribe la condición para que score aumente
    if(shrubsGroup.collide(kangaroo)){
      score = score + 1;
...
function reset(){
  ...
  kangaroo.changeAnimation = true;
  
  //destruye el grupo arbustos y obstáculos
  obstaclesGroup.destroyEach = true;
  shrubsGroup.destroyEach = true;
